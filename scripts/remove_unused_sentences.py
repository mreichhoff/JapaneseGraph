...
def get_average_frequency(sentences, word_frequencies):
    ...
    for sentence in sentences:
        words = set(sentence['ja']) - punctuation
...
def main():
    ...
    for item in character_and_word_set:
        ...
        for sentence in sentences:
            ...
            if item in sentence['ja'] or (len(item) == 1 and item in ''.join(sentence['ja'])) and 'en' in sentence:
                count = count + 1
                if(count > max_per):
                    break
                if ''.join(sentence['ja']) not in seen_sentences:
                    used_sentences.append(sentence)
                    seen_sentences.add(''.join(sentence['ja']))
        if count == 0:
            print(item)
        # if count < 5 and len(bad_examples) > 0:
        #     for i in range(0, min(len(bad_examples), (max_per - count))):
        #         if ''.join(bad_examples[i]['ja']) not in seen_sentences:
        #             used_sentences.append(bad_examples[i])
        #             seen_sentences.add(''.join(bad_examples[i]['ja']))
