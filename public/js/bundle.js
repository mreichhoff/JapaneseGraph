(function () {
    ...
    const firebaseConfig = {
        ...
        authDomain: "japanesegraph.firebaseapp.com",
    ...
    ***************************************************************************** */
    ...
    let updateVisited = function (nodes) {
        ...
        if (authenticatedUser) {
            ...
            const nodeRef = ref(db, `users/${authenticatedUser.uid}/visited/ja-JP/`);
            let updates = {};
            for (let i = 0; i < nodes.length; i++) {
    ...
    let saveStudyList = function (keys, localStudyList, isAddition) {
        if (authenticatedUser) {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                updates[`decks/ja-JP/${sanitizeKey(keys[i])}`] = localStudyList[keys[i]] || null;
                if (isAddition) {
                    //the user could've previously deleted the card...this addition should win out
                    updates[`deleted/ja-JP/${sanitizeKey(keys[i])}`] = null;
                }
            }
            update(nodeRef, updates).then(() => {
    ...
    let recordEvent = function (result) {
        ...
        if (authenticatedUser) {
            const db = getDatabase();
            const resultRef = ref(db, `users/${authenticatedUser.uid}/results/ja-JP/`);
            let updates = {};
    ...
    let fetchStudyResults = function () {
        if (canUpdateNonCriticalData(authenticatedUser, studyResultsLastUpdated)) {
            ...
            get(child(dbRef, `users/${authenticatedUser.uid}/results/ja-JP`)).then((snapshot) => {
    ...
    let fetchVisited = function () {
        if (canUpdateNonCriticalData(authenticatedUser, visitedLastUpdated)) {
            ...
            const dbRef = ref(getDatabase());
            get(child(dbRef, `users/${authenticatedUser.uid}/visited/ja-JP`)).then((snapshot) => {
    ...
    let initialize$4 = function () {
        ...
        onAuthStateChanged(auth, (user) => {
            if (user) {
                ...
                const db = getDatabase();
                const flashcardRef = ref(db, `users/${authenticatedUser.uid}/decks/ja-JP`);
